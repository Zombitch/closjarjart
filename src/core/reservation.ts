...
export async function computeNightPriceTotal(dateStart: Date, dateEnd: Date) {
  const config = await ConfigModel.findOne().sort({ createdAt: -1 }).lean();
  const nightlyRate = config?.price ?? 0;
  return computeNight(dateStart, dateEnd) * nightlyRate;
}
...
export async function getReservationsAsArray(fromNow: boolean = true) {
  ...
  return reservationSchemaList.map(reservation => [
    reservation.startDate.toISOString(),
    reservation.endDate.toISOString(),
    reservation._id as ObjectId,
    reservation.archived,
    reservation.type,
  ] as [string, string, ObjectId, boolean | null | undefined, string | null | undefined]);
}
...
export async function proceedReservation(
  startDate: Date,
  endDate: Date,
  guests: number,
  lastname: string,
  ...
  if (totalPrice < 0) {
    totalPrice = await computeTotalPrice(startDate, endDate);
  }
  ...
  if (await checkReservationConflict(startDate, endDate)) {
    ...
    return {
      ok: false as const,
      message: "Une autre personne a réservé ce créneau entre temps. Nous vous invitons à sélectionner d'autres dates.",
  ...
  const emailHtml = buildReservationRequestEmail({
    ...
    nights: nightNumber.toString(),
    price: totalPrice.toString(),
  } satisfies ReservationEmailParams);

  const emailSubject = `Location du ${startDateFR} au ${endDateFR}`;

  if (shouldSendEmail && env.mail.to) {
    await sendMail({
      to: env.mail.to,
      replyTo: email,
  ...
  return {
    ok: true as const,
    message: 'Votre demande de réservation a été envoyée. Vous serez contacté dans moins de 24h par l\'hôte pour valider la réservation.',
    reservations: reservationArray,
  };
}
